...
// 修改epollfd中指定文件描述符的监听事件（这里是追加 EPOLLONESHOT 状态）
...
Http_Connect::LINE_STATE Http_Connect::parse_line()
{
    ...
    for (; m_checked_idx < m_read_idx; m_checked_idx++)
    {
        ...
        if (temp == '\r')
        {
            ...
            {
                // 匹配到\r\n
                m_read_buf[m_checked_idx++] = '\0';  // 为什么要变成'\0'？
...
Http_Connect::HTTP_CODE Http_Connect::parse_request_line(char *text)
{
    ...
    // strpbrk() 函数将从 str1 中搜索并返回指向第一个在 str2 中出现的字符的指针。如果未找到匹配的字符，则返回 NULL。
    m_url = strpbrk(text, " \t");
    if (!m_url)
    {
        return BAD_REQUEST;  // 没有空格或者制表符，则语法错误
    }
    // GET\0/index.html HTTP/1.1
    *m_url = '\0';  // 把第一个空格或制表符（请求方法和URL之间的）改为'\0'，便于下面确定请求方法
    m_url++;
    ...
    // /index.html HTTP/1.1
    m_version = strpbrk(m_url, " \t");  // 把URL和协议版本之间的空格或制表符变为'\0'
    if (!m_version)
    {
        return BAD_REQUEST;
    }
...
Http_Connect::HTTP_CODE Http_Connect::process_read()
{
        ...
        || ((m_check_state == CHECK_STATE_CONTENT) && (line_state == LINE_OK)))
    {
        ...
        m_start_line = m_checked_idx;  //m_start_line变为下一行的起始地址？
...
bool Http_Connect::write()
{
    ...
    while (1)
    {
        ...
        // fd：表示文件描述符，指向要写入的目标文件。
        // iov：是一个指向 iovec 结构体数组的指针，每个 iovec 结构体描述了一个缓冲区的地址和长度。
        // iovcnt：表示 iovec 数组中元素的数量。
        // writev() 函数会按照 iovec 数组中的顺序将每个缓冲区
        // 的内容依次写入到文件描述符 fd 所代表的文件中
        // 返回写入的总字节数。
