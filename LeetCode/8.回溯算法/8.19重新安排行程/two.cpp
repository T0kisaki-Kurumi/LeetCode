...
public:
    vector<string> res;
    unordered_map<string, map<string, int>> fromto;
    bool backtracking(vector<string>& res, int len){
        if(res.size() == len) return true;
        string cur = res.back();
        for(auto& next: fromto[cur]){
            if(next.second > 0){
                --next.second;
                res.push_back(next.first);
                if(backtracking(res, len)) return true;
                res.pop_back();
                ++next.second;
            }
        }
    ...
    vector<string> findItinerary(vector<vector<string>>& tickets) {
        ...
        res.push_back("JFK");
        backtracking(res, tickets.size()+1);
        return res;
    }
};
