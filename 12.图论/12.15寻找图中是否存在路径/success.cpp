...
public:
    ...
    int find(vector<int>& father, int u){
        ...
        return u==father[u] ? u : find(father, father[u]); //不做路径压缩，否则秩就变了
    }
    ...
    void join(vector<int>& father, int u, int v, vector<int>& rank){
        u = find(father, u);
        v = find(father, v);
        if(u == v) return;
    ...
    bool validPath(int n, vector<vector<int>>& edges, int source, int destination) {
        vector<int> father(n);
        vector<int> rank(n, 1); //记录以每个节点为根的树的高度(秩)
        init(father);
        for(auto vec: edges){
            join(father, vec[0], vec[1], rank);
        }
